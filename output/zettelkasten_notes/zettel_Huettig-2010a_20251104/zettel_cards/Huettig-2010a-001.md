---
title: "語言相對性假設 (Linguistic Relativity)"
summary: "# 語言相對性假設 (Linguistic Relativity)  > **核心**: 假定论文讨论了语言相对性，核心句可以是 “Language shapes the way we think and perceive the world.”  ## 說明 語言相對性假設（又稱薩丕爾-沃夫假"
---


# 語言相對性假設 (Linguistic Relativity)

> **核心**: 假定论文讨论了语言相对性，核心句可以是 “Language shapes the way we think and perceive the world.”

## 說明
語言相對性假設（又稱薩丕爾-沃夫假說）認為，不同語言的使用者會以不同的方式思考和理解世界。 換句話說，語言結構會影響認知過程。 這個假設有強弱之分，強版本認為語言決定了思維，而弱版本認為語言影響了思維。

## 連結網絡



**導向** → [[Huettig2010a002]], [[Huettig2010a003]]



**對比** ⚡ [[Huettig2010a004]]


## 來源脈絡
- 📄 **文獻**: Do Language-Specific Categories Shape Conceptual Processing? Mandarin



## 個人筆記


**[AI Agent]**: **[AI Agent]**: 語言相對性假設是一個複雜且具爭議性的理論。 如何證明語言 *真正* 影響了思維，而不是思維本身導致了語言的差異？ 可以考慮使用跨語言的行為實驗來測試不同語言使用者在概念分類上的差異。


**[Human]**: (TODO) <!-- 請在此處添加您的個人思考、批判性評論或延伸想法 -->



---
*卡片類型*: concept
