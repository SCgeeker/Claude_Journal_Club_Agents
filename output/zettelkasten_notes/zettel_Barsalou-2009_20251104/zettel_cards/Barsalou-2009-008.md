---
id: Barsalou-2009-008
title: "行為預測（Behavioral Prediction）"
tags: [[預測], [行為], [社會認知]]
source: "Simulation, Situated Conceptualization, and Prediction" (None)
paper_id: 59
created: 2025-11-04
type: concept
---

# 行為預測（Behavioral Prediction）

> **核心**: [The ability to anticipate the actions and consequences of oneself and others.]

## 說明
行為預測是指預測自己和他人的行為以及這些行為可能產生的後果的能力。這種能力對於社會互動、合作和競爭至關重要。行為預測不僅依賴於對他人意圖和信念的理解，還依賴於對情境和過去經驗的記憶。

## 連結網絡


**基於** → [[Barsalou2009003]]



**相關** ↔ [[Barsalou2009002]]



## 來源脈絡
- 📄 **文獻**: Simulation, Situated Conceptualization, and Prediction



## 個人筆記


**[AI Agent]**: **[AI Agent]**: 如何讓AI系統具備行為預測的能力？ 是否可以通過觀察和模仿來學習人類的行為模式？ 行為預測在自動駕駛、社交機器人等領域有什麼應用前景？


**[Human]**: (TODO) <!-- 請在此處添加您的個人思考、批判性評論或延伸想法 -->



---
*卡片類型*: concept
