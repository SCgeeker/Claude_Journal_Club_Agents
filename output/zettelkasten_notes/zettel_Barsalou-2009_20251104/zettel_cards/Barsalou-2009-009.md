---
id: Barsalou-2009-009
title: "動態概念表徵（Dynamic Conceptual Representation）"
tags: [[概念], [表徵], [動態]]
source: "Simulation, Situated Conceptualization, and Prediction" (None)
paper_id: 59
created: 2025-11-04
type: concept
---

# 動態概念表徵（Dynamic Conceptual Representation）

> **核心**: [The idea that concepts are represented in a flexible and adaptable manner, changing based on context and experience.]

## 說明
動態概念表徵強調概念的表徵方式不是固定的，而是根據情境和經驗不斷調整的。這種調整可以涉及概念的激活水平、概念之間的連接強度以及概念的組成成分。動態概念表徵允許我們更靈活地使用概念，並根據需要調整其意義。

## 連結網絡


**基於** → [[Barsalou2009005]]


**導向** → [[Barsalou2009011]]




## 來源脈絡
- 📄 **文獻**: Simulation, Situated Conceptualization, and Prediction



## 個人筆記


**[AI Agent]**: **[AI Agent]**: 如何設計AI系統，使其能夠學習和使用動態概念表徵？是否可以利用注意力機制來調整概念表徵的激活水平？動態概念表徵在自然語言處理、圖像識別等領域有什麼應用價值？


**[Human]**: (TODO) <!-- 請在此處添加您的個人思考、批判性評論或延伸想法 -->



---
*卡片類型*: concept
