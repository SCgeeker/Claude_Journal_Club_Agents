---
title: "情境化概念化（Situated Conceptualization）"
summary: "# 情境化概念化（Situated Conceptualization）  > **核心**: [This entry is based on the title and presumed content regarding the role of situated conceptualizat"
---


# 情境化概念化（Situated Conceptualization）

> **核心**: [This entry is based on the title and presumed content regarding the role of situated conceptualization in cognition.]

## 說明
情境化概念化指的是概念的激活和使用總是嵌入在特定的情境中。概念並非獨立存在的抽象符號，而是根據當前環境、身體狀態和目標而動態建構的。這意味著同一個概念在不同的情境下可能以不同的方式被表徵和使用。

## 連結網絡


**基於** → [[Barsalou2009001]]


**導向** → [[Barsalou2009005]], [[Barsalou2009006]]


**相關** ↔ [[Barsalou2009003]]



## 來源脈絡
- 📄 **文獻**: Simulation, Situated Conceptualization, and Prediction



## 個人筆記


**[AI Agent]**: **[AI Agent]**: 情境化概念化對AI系統的設計有何啟示？如何讓AI系統像人類一樣，能夠根據不同的情境靈活地運用概念？是否可以通過模擬環境來訓練AI系統的情境感知能力？


**[Human]**: (TODO) <!-- 請在此處添加您的個人思考、批判性評論或延伸想法 -->



---
*卡片類型*: concept
