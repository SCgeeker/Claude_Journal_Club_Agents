---
title: "行動計劃干擾 (Action Planning Interference)"
summary: "If this simulation requires the same neural systems as the planning and guidance of real action, understanding a toward sentence should interfere with making a movement away from the body to indicate yes (yes-is-far), and understanding an away sentence should interfere with making a movement toward the body (yes-is-near)."
---

> **核心**: "If this simulation requires the same neural systems as the planning and guidance of real action, understanding a toward sentence should interfere with making a movement away from the body to indicate yes (yes-is-far), and understanding an away sentence should interfere with making a movement toward the body (yes-is-near)."


## 說明
此研究指出，如果理解句子所需的認知模擬使用了與實際行動計劃和執行相同的神經系統，那麼理解一個朝向身體的句子應該會干擾做出遠離身體的動作（“是”在遠處），反之亦然。 這種干擾效應支持了語言理解與行動之間共享神經資源的觀點。


## 連結網絡


**基於** → [[Glenberg-2002-003]]



## 來源脈絡
- 📄 **文獻**: PsychonomicBulletin&Review


## 個人筆記


**[AI Agent]**: **[AI Agent]**: 實驗結果支持了行動計劃干擾的假設，但需要進一步的研究來確定具體的神經機制。例如，可以使用神經影像技術來觀察在語言理解和行動計劃過程中，大腦中哪些區域被激活，以及它們之間的相互作用。


**[Human]**: (TODO) <!-- 請在此處添加您的個人思考、批判性評論或延伸想法 -->



---
*卡片類型*: concept
